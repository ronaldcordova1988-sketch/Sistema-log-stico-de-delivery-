import 'package:cloud_firestore/cloud_firestore.dart';

/// Modelo de Mantenimiento de Aceite con alerta a 200km
class MantenimientoAceiteModel {
  final String id;
  final String empleadoId;
  final DateTime fecha;
  final double precio;
  final double kmActual;
  final double kmProximoCambio;
  final bool liquidado;
  final String? corteId;
  final bool syncedToServer;

  MantenimientoAceiteModel({
    required this.id,
    required this.empleadoId,
    required this.fecha,
    required this.precio,
    required this.kmActual,
    required this.kmProximoCambio,
    this.liquidado = false,
    this.corteId,
    this.syncedToServer = false,
  });

  factory MantenimientoAceiteModel.create({
    required String empleadoId,
    required double precio,
    required double kmActual,
  }) {
    return MantenimientoAceiteModel(
      id: '',
      empleadoId: empleadoId,
      fecha: DateTime.now(),
      precio: precio,
      kmActual: kmActual,
      kmProximoCambio: kmActual + 5000, // Cambio cada 5000 km
      liquidado: false,
      syncedToServer: false,
    );
  }

  factory MantenimientoAceiteModel.fromFirestore(DocumentSnapshot doc) {
    final data = doc.data() as Map<String, dynamic>;
    return MantenimientoAceiteModel(
      id: doc.id,
      empleadoId: data['empleadoId'],
      fecha: (data['fecha'] as Timestamp).toDate(),
      precio: (data['precio'] as num).toDouble(),
      kmActual: (data['kmActual'] as num).toDouble(),
      kmProximoCambio: (data['kmProximoCambio'] as num).toDouble(),
      liquidado: data['liquidado'] ?? false,
      corteId: data['corteId'],
      syncedToServer: true,
    );
  }

  Map<String, dynamic> toFirestore() {
    return {
      'empleadoId': empleadoId,
      'fecha': Timestamp.fromDate(fecha),
      'precio': precio,
      'kmActual': kmActual,
      'kmProximoCambio': kmProximoCambio,
      'liquidado': liquidado,
      'corteId': corteId,
    };
  }

  Map<String, dynamic> toHive() {
    return {
      'id': id,
      'empleadoId': empleadoId,
      'fecha': fecha.toIso8601String(),
      'precio': precio,
      'kmActual': kmActual,
      'kmProximoCambio': kmProximoCambio,
      'liquidado': liquidado,
      'corteId': corteId,
      'syncedToServer': syncedToServer,
    };
  }

  factory MantenimientoAceiteModel.fromHive(Map<String, dynamic> data) {
    return MantenimientoAceiteModel(
      id: data['id'] ?? '',
      empleadoId: data['empleadoId'],
      fecha: DateTime.parse(data['fecha']),
      precio: (data['precio'] as num).toDouble(),
      kmActual: (data['kmActual'] as num).toDouble(),
      kmProximoCambio: (data['kmProximoCambio'] as num).toDouble(),
      liquidado: data['liquidado'] ?? false,
      corteId: data['corteId'],
      syncedToServer: data['syncedToServer'] ?? false,
    );
  }

  MantenimientoAceiteModel copyWith({
    String? id,
    bool? liquidado,
    String? corteId,
    bool? syncedToServer,
  }) {
    return MantenimientoAceiteModel(
      id: id ?? this.id,
      empleadoId: empleadoId,
      fecha: fecha,
      precio: precio,
      kmActual: kmActual,
      kmProximoCambio: kmProximoCambio,
      liquidado: liquidado ?? this.liquidado,
      corteId: corteId ?? this.corteId,
      syncedToServer: syncedToServer ?? this.syncedToServer,
    );
  }

  // ⭐ ALERTA A 200KM
  bool debeAlertarProximoCambio(double kmActualVehiculo) {
    final kmRestantes = kmProximoCambio - kmActualVehiculo;
    return kmRestantes <= 200 && kmRestantes > 0;
  }

  bool cambioVencido(double kmActualVehiculo) {
    return kmActualVehiculo >= kmProximoCambio;
  }

  double kmRestantes(double kmActualVehiculo) {
    return kmProximoCambio - kmActualVehiculo;
  }

  String getMensajeAlerta(double kmActualVehiculo) {
    final restantes = kmRestantes(kmActualVehiculo);
    if (restantes <= 0) {
      return '⚠️ ¡Cambio de aceite vencido!';
    } else if (restantes <= 200) {
      return '⚠️ Cambio de aceite en ${restantes.toInt()} km';
    }
    return 'Próximo cambio en ${restantes.toInt()} km';
  }

  bool get puedeEditar => !liquidado;
  bool get necesitaSync => !syncedToServer;
}
